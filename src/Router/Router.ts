 ...
 * @constructor
 * @param {function} [resolveFunc] used as parser for message
 *                                 to match with controllers.
 * @param {T} Controller Function Interface, supposed to be, for example,
 *                       (msg: chrome.runtime.ExtensionMessage) => any
 */
export class Router<T extends (msg: any) => any> {
   ...
   * @param {function} controllerFunc exacutable function
   *                                  when specified route is matched
   */
  ...
  /**
   * `listener` provides listener function
   *  which can satisfy Chrome EventListener Func.
  ...
  private listen(
      message: any,
      sender?: any,
      sendResponse = (response: any) => {/* do nothing */},
  ) {
    try {
      ...
      if (typeof response === 'undefined') {
        // If the Controller doesn't return anything,
        // it should NOT call `sendResponse`.
        // Mostly, Controllers for messages,
        // which can't receive response from background,
  ...
  /**
     * `match` resolves message to routing name,
     * and return registered controller.
     * @private
     * @param {any} message
     * @param {any} sender
  ...
  private match(message, sender) {
    ...
    if (!controllerFunc) {
      controllerFunc = constructor._NotFoundController;
    }
   ...
   * @return {{
   *  data: {message: string},
   *  error: {message: string, name: string, stack: any}
   * }}
   */
